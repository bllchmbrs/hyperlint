...
class RulesEditor(BaseEditor):
    ...
    def __init__(self, rules_directory=None, include_rules=None, exclude_rules=None, dry_run=None, **kwargs):
    ...
    def apply_rule(self, rule_content: str, rule_name: str) -> None:
        ...
        if self.config.dry_run:
            logger.info(f"Dry run: Found {len(issues)} issues for rule {rule_name}, but not applying changes")
            return
