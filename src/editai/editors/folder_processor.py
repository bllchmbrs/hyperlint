...
class FolderProcessor(BaseModel):
    ...
    def process_directory(self, dry_run: bool = False) -> Dict[Path, str]:
        ...
        search_pattern = (
            f"**/{self.include_pattern}" if self.recursive else self.include_pattern
        )
        file_paths = list(self.directory_path.glob(search_pattern))

        if not file_paths:
            logger.warning(
                f"No files found matching pattern '{self.include_pattern}' in {self.directory_path}"
            )
            return {}
        ...
        logger.info(
            f"Found {len(file_paths)} files to process in {self.directory_path}"
        )
        ...
        for file_path in file_paths:
            try:
                ...
                if not dry_run:
                    ...
                    logger.info(
                        f"Dry run - would update {file_path.relative_to(self.directory_path)}"
                    )
